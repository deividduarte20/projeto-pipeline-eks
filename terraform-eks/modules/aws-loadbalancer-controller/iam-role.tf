...
resource "aws_iam_role" "this" {
  name = "${var.project_name}-loadbalancer_controller"
    ...
    "Statement": [
        {
            ...
            "Condition": {
                "StringEquals": {
                    "oidc.eks.${data.aws_region.this.name}.amazonaws.com/id/${local.oidc}:sub": "system:serviceaccount:kube-system:aws-load-balancer-controller",
...
EOF
  ...
  tags = {
    Name = "${var.project_name}-loadbalancer-controller-role"
  }
}
